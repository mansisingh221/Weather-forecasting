...
class Thundersttorm(Weather):
    ...
    def load_icon(self, icon_id, w_desc, org):
        ...
        canvas.create_text(150, 220, text='Current weather in: {}, {}:'.format(org['city'] , org['country']), font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 250, text='Sky: ' + str(org['sky']), font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 280, text='Maximum Temperature: ' +  str(org['temp_max']) + '\xb0', font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 310, text='Minimum Temperature: ' +  str(org['temp_min']) + '\xb0', font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 340, text='Wind Speed: {} m/s, Degree: {} \xb0'.format(org['wind'], org['wind_deg']), font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 370, text='Humidity: {}'.format(org['humidity']) + '%', font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 400, text='Visibilty: ' + str(org['visibility']) + 'meter', font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 430, text='Clouds: {}'.format(org['cloudiness']) + '%', font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 460, text='Pressure: {}'.format(org['pressure']) + 'hPa', font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 490, text='Sunrise at: {}'.format(org['sunrise']), font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 510, text='Sunset at: {}'.format(org['sunset']), font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 540, text='Longitude: ' + str(org['longitude']) + '\xb0', font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 570, text='Latitude: ' + str(org['latitude']) + '\xb0', font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 600, text='Weather ID: {}'.format(org['typeid']), font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 630, text='Last update from the server: {}'.format(org['dt']), font=('Calibri', -20, 'bold'))
        canvas.create_text(150, 660, text='Weather Type: {}'.format(org['wtype']), font=('Calibri', -20, 'bold'))
...
thunder1 = Thundersttorm()
url=thunder1.url_builder(cityName)
fetched_data=thunder1.data_fetch(url)
org=thunder1.data_organizer2(fetched_data)
thunder1.data_output(org)
weatherid=('{}'.format(org['typeid']))
icon_id = ('{}'.format(org['icon']))
w_desc=('{}'.format(org['wtype']))
#thunder1.load_icon(icon_id, thunder1.data_output(org))
thunder1.load_icon(icon_id, w_desc, org)
